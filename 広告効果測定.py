...
# ページ設定
st.set_page_config(page_title="効果測定ツール", layout="wide")
...
def create_query(anken_id_val, shop_id, item_id, easy_id_list, start_date, end_date):
    ...
    # end_dateは使用せず、開始日の1ヶ月後を計算に使用
    
    # 日付計算ロジック
    sale_end_date = (datetime.strptime(end_date, '%Y-%m-%d') + timedelta(days=1)).strftime('%Y-%m-%d')
...
st.title("📊 広告効果測定ツール (BigQuery連携)")
st.markdown("Excelファイルをアップロードすると、BigQueryで集計を行い、ROASを計算して結果を出力します。")
...
if uploaded_file is not None:
    # 実行ボタン
    if st.button("集計開始", type="primary"):
        status_text = st.empty()
        progress_bar = st.progress(0)
        
        try:
            # 1. データ読み込み
            status_text.info("ファイルを読み込み中...")
            ...
            st.write(f"対象グループ数: {total_groups}")

            # 3. BigQueryクライアント接続
            client = bigquery.Client(project=project_id)

            # 4. 並列実行
            status_text.info(f"BigQueryで集計中... (並列数: {MAX_WORKERS})")
            all_results = []
            ...
            # 5. 結果結合
            if not all_results:
                st.error("結果が0件でした。条件に合致するデータがありません。")
            else:
                status_text.info("結果を出力中...")
                ...
                status_text.success("集計完了！以下のボタンからダウンロードしてください。")
